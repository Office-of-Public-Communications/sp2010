...
window.addEventListener('load', function(event) {
  ...
  var charLimit = 4800;
  var maxBSLimit = 9600;
  var sumsum = 0; // array limited to 5000 chars || 100 items for ms translate

  var bigArr = [];
  bigArr.length = 0; // a ginormous array for insane cases..

  var biggerArr = [];
  biggerArr.length = 0;
  var sumSumArray = [];
  sumSumArray.length = 0; // console.log(bigArr, biggerArr, sumSumArray)
    ...
    for (var i = 0; i < filteredParents.length; i++) {
      ...
      if (chars > charLimit) {
        ...
        for (var b = 0; b < breaks.length; b++) {
          firstCut += breaks[b]; // console.log(firstCut);
          // console.log(b);

          var rb = filteredParents[i].transarray; // console.log(b != 0 && b == breaks.length)
          // take care of cases where there's only one break

          if (breaks.length == 1) {
            var first = textArray.slice(0, breaks[0]).split('[')[1];
            rb.push(first);
            var last = textArray.slice(firstCut).split(']')[0]; // console.log(last)
          ...
          if (breaks.length != 1) {
            if (b == 0) {
              var first = textArray.slice(0, breaks[0]).split('[')[1];
              rb.push(first); // console.log('first: ' + first);
            } else if (b != 0 && b == breaks.length - 1) {
              var last = textArray.slice(firstCut).split(']')[0]; //console.log('last' + textArray.slice(firstCut));

              rb.push(last); // console.log('last: ');
            } else if (b != breaks.length - 1) {
              var inner = textArray.slice(firstCut, firstCut + breaks[b + 1]);
              rb.push(inner); // console.log('inner: ' + inner);
            } else {
              // console.log(b)
      ...
      if (chars < charLimit) {
        sumsum += chars; // console.log(chars, sumsum);
        ...
        if (sumsum >= charLimit && bigArr.length < arrLimit) {
          ...
          sumSumArray = [];
          sumsum = chars; // console.log('new sum')
          //   console.log(sumsum);
  ...
  $(document).ready(function() {
    $('a.trBt').click(function() {
      var lan = $(this).data('lang'); //console.log(lan == 'en');

      if (lan !== 'en') {
        var getTranslation = async function getTranslation() {
          // fetch translations
          console.log(bigArr);
          ...
          for (var i = 0; i < bigArr.length; i++) {
            ...
            var translations = await fetch(
              ...
              '&textType=html', {
                ...
                headers: {
                  'Content-Type': 'application/json; odata=verbose',
                  'Ocp-Apim-Subscription-Key': '937e78fa81e94398aa55b2cfc814183d' //"Access-Control-Allow-Origin": "https://broward.org"
                }
              }
            );
            var jsons = await translations.text(); // console.log(jsons);

            tresults = JSON.parse(jsons); // console.log(tresults);
          ...
          var flatArr = bigArr.flat(2); //console.log(flatArr);
          ...
          for (var i = 0; i < filteredParents.length; i++) {
            ...
            if (filteredParents[i].transarray.length > 0) {
              for (var j = 0; j < filteredParents[i].transarray.length; j++) {
                ...
                ttresults = JSON.parse(jsons); // console.log(ttresults);
        ...
        var replaceText = async function replaceText() {
...
$('#translate').on('click', function(e) {
  ...
  e.stopPropagation(); //  $(this).find("ul.tr-dd").toggleClass("");
});
